import{c as g,a as A}from"./assets/scroll-9bed19a6.js";import{i as c,a as f,P as _}from"./assets/vendor-ebe44317.js";const b=document.querySelector(".quote__backend"),$=new Date().toLocaleDateString();async function D(){b.innerHTML=`
    ...
    `;try{return(await f.get("https://energyflow.b.goit.study/api/quote")).data}catch{c.error({title:"Error",message:"Something went wrong, try again"})}}function S(e,t){return`<p class="quote__backend-text">${t}</p>
            <p class="quote__backend-author">${e}</p>`}function R(){if(localStorage.getItem("quote")&&localStorage.getItem("savedDate")===$){const{author:e,quote:t}=JSON.parse(localStorage.getItem("quote"));b.innerHTML=S(e,t)}else D().then(({author:e,quote:t})=>{const a={author:e,quote:t};localStorage.setItem("quote",JSON.stringify(a)),localStorage.setItem("savedDate",$),b.innerHTML=S(e,t)}).catch(e=>c.error({title:"Error",message:e.message}))}R();const j=document.getElementById("subscriptionForm"),M=document.getElementById("email"),z=new RegExp(/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/);j.addEventListener("submit",async function(e){e.preventDefault();try{z.test(M.value)?await f.post("https://energyflow.b.goit.study/api/subscription",{email:M.value}).then(t=>c.success({title:"Success",message:t.data.message})).catch(t=>c.error({title:"Error",message:t.response.data.message})):c.error({title:"Error",message:"Please enter the correct email!"})}catch{c.error({title:"Error",message:"Something went wrong, try again"})}});const w="/js_96_FunnyBilbur/assets/exercises-sprite-d4ab7b8a.svg";function x(e,t){const a=document.getElementById("tui-pagination-container"),s=t<3?t:3,r={totalItems:e*t,itemsPerPage:e,visiblePages:s,centerAlign:!0},i=new _(a,r);return s<=1?a.style.display="none":a.style.display="block",i}const o=document.querySelector(".exercises-list");let u="Muscles",y=1;const F=12,O=9;let E,L;const v=document.getElementById("exercises-search-form"),l=document.querySelector(".exircises-category"),k=document.querySelector(".exercises-title"),Q=v.querySelector(".exercises-search-input");async function m(e,t){let a=new URLSearchParams({filter:e,page:t,limit:F});try{await f.get(`https://energyflow.b.goit.study/api/filters?${a}`).then(s=>{let r=s.data.results;E=s.data.totalPages;const i=r.map(({name:n,filter:d,imgUrl:h})=>`<li class="exercises-list-item" data-name="${n}" data-filter="${d}">
